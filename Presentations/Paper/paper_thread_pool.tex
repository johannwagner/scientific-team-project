...
\textbf{TODO: maybe table of provided functions} \\
We define the processing of tasks as a procedure of 3-4 steps. In Figure 2, we show the flow of task enqueueing and execution in the thread pool system. In the first step, tasks are passed as a parameter to the enqueue function provided by the thread pool. The tasks have to be created before passing and later on freed after processing by the calling function.  We provide different functions to enqueue a single task or an array of tasks. 
